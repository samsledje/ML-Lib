...
class FeedForwardNet:
    ...
    def train(self, X, y, epochs=5000):
        for i in range(epochs):
            # Forward Step
            hidden_activations = sigmoid(np.dot(X, self.hidden_weights) + self.hidden_biases)
            output_activations = sigmoid(np.dot(hidden_activations, self.output_weights) + self.output_biases)

            # Back Propagation
            errors = y - output_activations
            output_deriv = dev_sigmoid(output_activations)
            hidden_deriv = dev_sigmoid(hidden_activations)
